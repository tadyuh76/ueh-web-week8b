                    ...
                    <form method="POST">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="ten" name="ten" 
                                   value="<?= $this->escape($product['ten']) ?>" required>
                        </div>
                        
                        <div class="mb-3">
                            <label for="mota" class="form-label">Mô tả:</label>
                            <textarea class="form-control" id="mota" name="mota" rows="3"><?= $this->escape($product['mota']) ?></textarea>
                        </div>
                        
                        <div class="mb-3">
                            <label for="gia" class="form-label">Giá (VND):</label>
                            <input type="number" class="form-control" id="gia" name="gia" 
                                   value="<?= $product['gia'] ?>" min="0" step="1000" required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="id_dm" name="id_dm" required>
                                ...
                                <?php foreach ($categories as $cat): ?>
                                    <option value="<?= $cat['id'] ?>" <?= ($cat['id'] == $product['id_dm']) ? 'selected' : '' ?>>
                                        <?= $this->escape($cat['ten']) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="url" class="form-control" id="image_url" name="image_url" 
                                   value="<?= $this->escape($product['image_url'] ?? '') ?>"
                                   placeholder="https://example.com/image.jpg">
                            <small class="text-muted">Nhập URL hình ảnh sản phẩm (tùy chọn)</small>
                            <div id="image-preview" class="mt-2">
                                <?php if (!empty($product['image_url'])): ?>
                                    <img src="<?= $this->escape($product['image_url']) ?>" alt="Current image" 
